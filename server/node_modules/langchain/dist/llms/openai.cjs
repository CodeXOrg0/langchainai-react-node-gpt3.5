...
class OpenAI extends base_js_1.BaseLLM {
    ...
    async _generate(prompts, stop) {
        const subPrompts = (0, index_js_1.chunkArray)(prompts, this.batchSize);
        const choices = [];
        const tokenUsage = {};
        if (this.stop && stop) {
        ...
        const generations = (0, index_js_1.chunkArray)(choices, this.n).map((promptChoices) => promptChoices.map((choice) => ({
            text: choice.text ?? "",
            generationInfo: {
