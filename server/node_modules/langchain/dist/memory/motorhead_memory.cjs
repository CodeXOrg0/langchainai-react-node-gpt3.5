...
class MotorheadMemory extends chat_memory_js_1.BaseChatMemory {
    ...
    async init() {
        const res = await this.caller.call(index_js_1.fetchWithTimeout, `${this.motorheadURL}/sessions/${this.sessionId}/memory`, {
    ...
    async loadMemoryVariables(_values) {
        ...
        const result = {
            [this.memoryKey]: (0, base_js_1.getBufferString)(this.chatHistory.messages),
        };
        return result;
    }
    async saveContext(inputValues, outputValues) {
        await Promise.all([
            this.caller.call(index_js_1.fetchWithTimeout, `${this.motorheadURL}/sessions/${this.sessionId}/memory`, {
                timeout: this.timeout,
                method: "POST",
                body: JSON.stringify({
                    messages: [
