...
class SqlToolkit extends base_js_1.Toolkit {
    constructor(db) {
        ...
        this.db = db;
        this.tools = [
            new index_js_1.QuerySqlTool(db),
            new index_js_1.InfoSqlTool(db),
            new index_js_1.ListTablesSqlTool(db),
            new index_js_1.QueryCheckerTool(),
        ];
    }
}
...
function createSqlAgent(llm, toolkit, args) {
    ...
    const chain = new index_js_2.LLMChain({ prompt, llm });
    const agent = new index_js_3.ZeroShotAgent({
        llmChain: chain,
