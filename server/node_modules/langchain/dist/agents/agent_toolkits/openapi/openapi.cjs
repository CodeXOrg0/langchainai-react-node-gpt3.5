...
const index_js_1 = require("../../tools/index.cjs");
...
class RequestsToolkit extends base_js_1.Toolkit {
    constructor(headers) {
        ...
        this.tools = [new index_js_1.RequestsGetTool(headers), new index_js_1.RequestsPostTool(headers)];
    }
}
...
function createOpenApiAgent(llm, openApiToolkit, args) {
    ...
    const prompt = index_js_3.ZeroShotAgent.createPrompt(tools, {
        prefix,
        suffix,
        inputVariables,
    });
    const chain = new index_js_2.LLMChain({
        prompt,
        llm,
    });
    const toolNames = tools.map((tool) => tool.name);
    const agent = new index_js_3.ZeroShotAgent({ llmChain: chain, allowedTools: toolNames });
