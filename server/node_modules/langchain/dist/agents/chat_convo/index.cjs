...
const index_js_2 = require("../../prompts/index.cjs");
const template_js_1 = require("../../prompts/template.cjs");
const prompt_js_1 = require("./prompt.cjs");
const index_js_3 = require("../../schema/index.cjs");
class ChatConversationalAgentOutputParser extends index_js_3.BaseOutputParser {
    async parse(text) {
...
class ChatConversationalAgent extends agent_js_1.Agent {
    ...
    constructScratchPad(steps) {
        ...
        for (const step of steps) {
            thoughts.push(new index_js_3.AIChatMessage(step.action.log));
            thoughts.push(new index_js_3.HumanChatMessage((0, template_js_1.renderTemplate)(prompt_js_1.TEMPLATE_TOOL_RESPONSE, "f-string", {
    ...
    static createPrompt(tools, args) {
        ...
        const messages = [
            index_js_2.SystemMessagePromptTemplate.fromTemplate(systemMessage),
            new index_js_2.MessagesPlaceholder("chat_history"),
            index_js_2.HumanMessagePromptTemplate.fromTemplate(finalPrompt),
            new index_js_2.MessagesPlaceholder("agent_scratchpad"),
        ];
        return index_js_2.ChatPromptTemplate.fromPromptMessages(messages);
    }
    static fromLLMAndTools(llm, tools, args) {
        ChatConversationalAgent.validateTools(tools);
        const prompt = ChatConversationalAgent.createPrompt(tools, args);
        const chain = new index_js_1.LLMChain({ prompt, llm });
