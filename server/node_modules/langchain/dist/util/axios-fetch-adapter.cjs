...
const parse_1 = require("@fortaine/fetch-event-source/parse");
/**
...
async function getResponse(request, config) {
    ...
    if (stageOne.status >= 200 && stageOne.status !== 204) {
        if (config.responseType === "stream") {
            ...
            if (!contentType?.startsWith(fetch_event_source_1.EventStreamContentType)) {
                throw new Error(`Expected content-type to be ${fetch_event_source_1.EventStreamContentType}, Actual: ${contentType}`);
            }
            await (0, parse_1.getBytes)(stageOne.body, (0, parse_1.getLines)((0, parse_1.getMessages)(config.onmessage)));
        }
        else {
...
function createRequest(config) {
    ...
    // for streaming
    if (config.responseType === "stream") {
        options.headers.set("Accept", fetch_event_source_1.EventStreamContentType);
    }
