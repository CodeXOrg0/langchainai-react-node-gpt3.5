...
const index_js_2 = require("../util/index.cjs");
...
class ChatVectorDBQAChain extends base_js_1.BaseChain {
    ...
    static async deserialize(data, values) {
        ...
        const serializedCombineDocumentsChain = await (0, index_js_2.resolveConfigFromFile)("combine_documents_chain", data);
        const serializedQuestionGeneratorChain = await (0, index_js_2.resolveConfigFromFile)("question_generator", data);
        return new ChatVectorDBQAChain({
            combineDocumentsChain: await base_js_1.BaseChain.deserialize(serializedCombineDocumentsChain),
            questionGeneratorChain: await llm_chain_js_1.LLMChain.deserialize(serializedQuestionGeneratorChain),
            k: data.k,
            vectorstore,
        });
    }
    ...
    static fromLLM(llm, vectorstore, options = {}) {
        ...
        const question_generator_prompt = index_js_1.PromptTemplate.fromTemplate(questionGeneratorTemplate || question_generator_template);
        const qa_prompt = index_js_1.PromptTemplate.fromTemplate(qaTemplate || qa_template);
